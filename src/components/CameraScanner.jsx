...
export default function CameraScanner({ open, onClose, onSave, showToast }) {
    ...
    const handleScan = useCallback(async () => {
        // Guard: evitar doble-clic que gastaría créditos dobles
        if (scanning) return
        ...
        try {
            ...
            setChecked(result.items.map((_, i) => i))   // todos seleccionados
            setPhase('result')
        } catch (e) {
            console.error(e)
            // Si no hay internet, informar. Si no, mensaje genérico
            // (el rate limit 429 ya se maneja con retry automático en gemini.js)
